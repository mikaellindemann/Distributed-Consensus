...
\label{chap:connecting-histories}
	...
	A subgraph of the history with the same ordering and reachability, but as few edges as possible, called a \textit{minimum equivalent graph}, is desired. If there is a path from an edge $x$ to an edge $y$ in graph G, there must also be a path from $x$ to $y$ in the transitive reduction of $G$, and vice versa. 
	...
	\newpar Why this is the case is described in the following chapters.
