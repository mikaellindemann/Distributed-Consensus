...
%	Furthermore DCR graphs specify several types of directed edges in the graph, which provide useful information for determining order of execution.
	...
	\todo[inline]{Til Søren: da du henviste os til \cite{hildebrandt2011safe}, var det så Projection og Compositions du vil have os til at se på? Vi har lidt svært ved at se hvordan vi skal bruge denne del i resten af rapporten. (Vi har også lidt svært ved at forstå det)}
	...
	\end{quotation}
	\figuretodo{Man kunne indsætte et flowchart her. Jeg (Mikael) synes ikke det er nødvendigt.}
...
%	The implementation of a distributed system functioning as a DCR graph used in this project does not support the milestone relation.
	...
	A \textit{directed graph} is a special case of the graph data structure, which consists of \textit{nodes} connected by \textit{edges}. In directed graphs an edge is a ordered pair of nodes, where the first element is the start node and the second is the end node. When traversing the graph it is only possible to traverse edges from their start node to their end node. There exists a \textit{path} from a node to another if it is possible to get from the first to the second node.
	...
	A \textit{topological order} of a directed graph is a linear ordering of the nodes in the graph, such that for every edge $a,b$ from node $a$ to $b$, $a$ comes before $b$.
	

	\section{Distributed Systems}
		Distributed systems is an area of computer science which focuses on the concepts, problems and design of computer systems where message transmission among processes are not negligible compared to the time between events in a single process. Traditionally these systems are distributed on computers in a network \cite{Coulouris:2011:DSC:2029110}. \todo[inline]{Tilføj cite til chapter eller bare hele bog? Jeg (Mikael) er ret sikker på at denne beskrivelse kommer fra Lamports artikel om ordering af events og ikke fra Distributed Systems bogen.}
		...
		Traditionally consensus in distributed systems is the process of reaching agreement among the participants in the system on a proposed value. This is often achieved by using an election algorithm such as Paxos\cite{Lamport:1998:PP:279227.279229}.\todo[inline]{Be precise about PAXOS failure modes. Then conclude that it cannot handle malicious agents because it does not handle byzantine failures.} The Paxos algorithm cannot handle corrupted agents, that is agents that collude, lie, or otherwise try to subvert the protocol, and can therefore not be used in this project. Furthermore election algorithms requires all agents of the election to be able to propose a single value which should be agreed upon, but since each DCR event does not know nor is able to produce the entire history of the workflow, and therefore cannot propose a single value, regular election algorithms cannot be used.
		
		\todo[inline]{There are consensus algorithms that are not election algorithms. Are you confusing the two terms?}
        
        \newpar We generalise the concept of consensus from being agreement on a single value to finding a value compatible with all proposed values. We will use this generalisation of consensus in \autoref{chap:order-of-execution}.\todo[inline]{Søren: (Writing that down, I can't help wondering if somebody didn't do that before?). Mikael: Jeg har prøvet at søge en halv times tid omkring dette og har ikke fundet noget brugbart.. :-(}
	...
	\section{Distributed DCR implementation}
		...
		\newpar At the time of implementation, only Condition, Response, Inclusion, and Exclusion relations were required.\todo[inline]{Skulle man lige tilføje en note om at der findes flere relationer? Ellers virker denne sætning ligegyldig.}
