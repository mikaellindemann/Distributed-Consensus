...
\chapter{Connecting Histories}\label{chap:connecting-histories}
    ...
    \newpar Unfortunately, central server systems have drawbacks. E.g. if the server crashes then it is not possible to acquire the addresses of any of the events and the workload.
	...
	Therefore it is desired to develop an algorithm that, given an event with the desired reachability, is able to recursively gather all the local histories of the workflow. Since DCR graphs can contain cycles the algorithm also has to avoid endless recursions.\todo{Det er underligt at denne sætning starter med therefore når den foregående siger noget om en drawback der ikke gøres noget ved.}
