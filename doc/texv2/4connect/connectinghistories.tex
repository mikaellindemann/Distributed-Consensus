...
\label{chap:connecting-histories}
    ...
    Given a set of local histories, we would like to connect the actions of the histories such that the result has the fewest possible concurrent actions, while requiring that for every history, B, that was part of the creation of history A, for any path from one action to another in B, this path must also exist in history A.\todo[inline]{Mikael: Jeg tror det skal gøres tydeligere at B er en lokal historik og A er den globale. Og så er det lidt underligt at B defineres før A.}\figuretodo{Jeg ved ikke om det er for komplekst at starte med. Men man kan godt lave en figur der præsenterer det ønskede resultat.}
    	\todo[inline]{Det ovenstende er skrevet på basis af Sørens kommentar, at han gerne ville se en "Hvad er målet, og hvordan laver vi de trin der opnår målet" inden vi beskrev dem. Det er derfor det virker lidt "omvendt".}
    ...
    \figuretodo{Figur af nogle events der matcher og nogen der ikke matcher - med timestamps of hele mollevitten}
	...
	\figuretodo{Figur med før efter merge af to historikker.}
    ...
    \figuretodo{måske figur af en client der kontater en server der har pile ud til alle events}
	...
	\figuretodo{måske en figur af en dcr graph med nogle ekstra pile der viser algoritmens gang}
	...
	\figuretodo{måske figur der viser besværligheden ved at nogle dele af graphen nås gennem en "evil"}