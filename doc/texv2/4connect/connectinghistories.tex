...
\label{chap:connecting-histories}
	After determining how to represent histories, we want to combine multiple histories into one history by using the information on actions. This chapter defines the this merging of actions. \todo{jeg forstår ikke sidste sætning}
    ...
    \todo[inline]{jeg tror faktisk jeg godt kan se at vi måske mangler lidt noget argumentation for hvorfor vi gør det her. Altså skrive at at hvert event kun har sin egen totalle orden og vi bliver derfor nød til at have noget der markerer orderner mellem dem. Og eftersom at actions er gemt kan disse bruge til at etablere happens before relations.}
    ...
    \newpar
    The concept of happens-before relations, as described in \autoref{subsec:orderingofevents} helps determining which actions have happened before others across events. \todo{ved ikke om den her sætning er nødvendig}
	...
	Actions should therefore be matched on more than just their corresponding action type. An action includes the ID and timestamp of both the performing and counterpart \todo{Fisk: Det er lidt som om at vores performer/receiver-concept er væk her?} event to ensure that matches are unique.
    ...
    \begin{figure}[H]
		...
		\end{minipage}
		\end{figure}
    \todo{figur 4.2 skal lige retets så den faktisk er valid. ligenu er countertimestamp ikke højere en local timestamp på E1}
