...
\chapter{Introduction}
	...
	Since DCR graphs allow infinite behaviour, the workflow can be in the same state multiple times. The previous identical states and the events that occurred in order to get to that state can be extracted from an order of execution \todo{Søren had a comment for clarification, and we've fixed it somewhat.}. Finally, for DCR graphs shared between two or more companies, one of the participating companies might want to check retroactively whether the other companies have followed the rules of the workflow.
