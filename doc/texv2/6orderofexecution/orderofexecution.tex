...
\section{Order of Execution}
	...
	\newpar An example of collapsing executions can be found in \autoref{fig:validation:collapsing}. On the figure each execution gets collapsed to a single entity with the same happens before relations as the original history. 
	
	The collapsing algorithm can be be seen on \ref{alg:collapse}, as it can be seen this algorithm starts out gathering all the actions of an execution and only after the single entity is created adds all the happens before relations, though different in approach this produces the same result.
