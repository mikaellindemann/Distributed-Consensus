...
\todo[inline]{Fisk: Måske smelt de her to mini-paragraffer sammen til én. Jeg synes det fungerer godt som intro, så.}
	...
	One approach to finding a single representation of an execution could be to examine if a single action of the execution could describe all the relations to all the other executions. Doing this involves filtering all actions not chosen to be the representative of the execution, and since filtering nodes remove happen-before relations, increasing the connectivity of the graph before filtering would be necessary.
	...
	After having found the transitive closure of the graph, actions that are either not in between \textit{Execute start} and \textit{Execute finish} action types can be filtered away from the graph. One of these execution types should denote an entire execution. This does not affect the order of the executions, since the execute action types are now connected after having found the transitive closure.
	...
	Compare the result to the result of using the \textit{Collapse} approach in \autoref{fig:problem:collapse}, where it can be seen that is is fact possible to determine that these actions happens in order. \textit{Collapse} is described below.
