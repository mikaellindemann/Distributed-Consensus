...
\chapter{Validation of Histories}\label{chap:validation}
	...
	An example of two events not having matching actions in their histories because of the timestamps can be seen \autoref{fig:agreement-on-original-timestamps}.
	
	\newpar To validate for this kind of cheating, pairs of local histories where the events have relations to eachother is examined. each ingoing action is matched with outgoing actions and if it is not possible to find a match, one of the events must be malicious. 
	
	\begin{ruledef}
		\textbf{Agreement on the original amount of executions}: The history of a malicious event states that it has executed either fewer or more times than it has done.		
	\end{ruledef}
	...
	\begin{ruledef}
		\textbf{Agreement on the original amount of counterpart executions}: The history of a malicious event states that it has been affected either fewer or more times than what has happened.
	\end{ruledef}
