...
\chapter{Validation of Histories}\label{chap:validation}
	...
	\newpar \textbf{Valid relations}: The local history of an event must only contain outgoing actions to events it has relations to in the workflow. Similarly the local history of an event must only contain incoming actions from events that has relations to it in the workflow.\todo{Sørg for at dette er defineret således at typerne skal være klare i forhold til historikken.}
	
	\figuretodo{figur der viser en action i en execution der ikke skal være der - og en anden der har en ingoing der ikke skal være der.}
	The validation of valid relations can be done by examining every action in the local history, and for each outgoing and incoming action, the counterpart and action type must be present in the rules of the workflow.
	...
	\todo[inline]{The definition of consistent cheating waz here}
