...
\chapter{Validation of Histories}\label{chap:validation}
	...
	\newpar Recall that the definition of inconsistent cheating specified that actions could be added, removed or changed and the resulting history must violate at least one of the properties 2, 3 and 4 of valid histories. To argue that the rules 1 through 8 cover the adding, removing and changing of actions in such ways each of these scenarios are examined:
	
	\begin{itemize}
		\item \textbf{Adding actions}: 
		\item \textbf{Removing actions}:
		\item \textbf{Changing actions}:
	\end{itemize}
