...
%	\newpar One could argue the implications of consistent cheating in sub-graphs such as those of case 1 and 2, since their these events have little consequences on the rest of the workflow state. Therefore these events can only exist in two states (before and after they are executed) and therefore not being able to handle these kinds of situations is not that big a flaw. The major problems are apparent in situation 6 and 7 where, for example, a company can own a sub-part of the graph with many events working together to create a falsified image of what has actually happened. 
	...
	\begin{table}[]
		...
		\begin{tabular}{lll}
			...
			Rule 1 (Only specified relations)         & Y          & N            \\
			Rule 2 (Only complete executions)         & Y          & N            \\
			Rule 3 (Executions only in valid states)  & Y          & N            \\
			Rule 4 (Only non disrupted executions)    & Y          & Y            \\
			Rule 5 (Wait for complete execution)      & Y          & Y            \\
			Rule 6 (Synchronous message exchange)     & Y          & Y            \\
			Rule 7 (Total order of timestamps)        & Y          & Y            \\
			Rule 8 (Message exchange timestamp order) & Y          & Y           
		\end{tabular}
	\end{table}
	...
	\begin{table}[]
		...
		\label{my-label}
		\begin{tabular}{llll}
			Consistent Cheating & Agreement on original timestamps & Agreement on amount of executions (incoming) & Agreement on amount of executions (outgoing) \\
			Case 1              & Not observable                   & Not observable                               & Not observable                               \\
			Case 2              & Not observable                   & Not observable                               & Not observable                               \\
			Case 3              & Not observable                   & Not observable                               & Not observable                               \\
			Case 4              & Not observable                   & Not observable                               & Not observable                               \\
			Case 5              & Not observable                   & Not observable                               & Not observable                               \\
			Case 6              & Not observable                   & Not observable                               & Not observable                              
		\end{tabular}
	\end{table}
	\todo{write about it}
