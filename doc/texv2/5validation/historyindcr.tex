...
\todo[inline]{Wind: Jeg er ret sikker på at der var nogle lange "Søren-kommentarer" her vi ikke har håndteret.}
\label{chap:consensusindcr}
	...
	\end{definition}
	\todo[inline]{Fisk: Der mangler en rigtig definition. Jeg er ret sikker på, at Søren ville have en bedre definition af hvad "cheating" egentlig er. Mikael: Vi har ændret definitionen til at være mere præcis siden vi afleverede sidst.}
	...
	\begin{algorithm}[H]
		\begin{algorithmic}
			\Function{UpdateRelation}{state, event, relationType}
			...
			\Function{Execute}{state, DCRRules, execution}
				...
				\State state' $\leftarrow$ \Call{SetExecutedTrue}{state, execution}
				...
				\If{\Call{IsExecutable}{state, conditions, execution}}
					\State \Return Success state'
				\Else
					\State \Return Failure state' 
				\EndIf
			\EndFunction
	...
	\end{algorithm}
	\todo{Check lige op på algoritmen generelt}
