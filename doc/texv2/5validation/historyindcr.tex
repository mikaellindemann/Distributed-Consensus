...
\label{chap:consensusindcr}
	...
	\todo[inline]{figur der viser en action i en execution der iokke skal være der - og en anden der har en ingoing der ikek skal være der.}
	...
	\todo[inline]{Figur hvor der ikke er 2 outgoing selvom den burde}
	\todo[inline]{figur hvor der ikke er 2 ingoing selvom den burde}
	...
	\todo[inline]{en figur der viser en ingoing inde i en execution}
	...
	\todo[inline]{Figur der viser en der bliver påvirket af en anden ind imellem at den blvier påvirker af en anden.}
	...
	\todo[inline]{udvid figur til at vise hvordan hvilke situationer der kan opstå i alle tre tilfælde}
    \begin{figure}[H]
		\centering
	...
	\todo[inline]{to historikker der er ens bortset fra timestamps}
	...
	\todo[inline]{to historikker hvor antallet af executions ikke matcher}
	...
	\todo[inline]{to historikker hvor antallet af incoming groups ikke matcher }
	...
	\todo[inline]{figur eksempler til tekst, der viser hvordan at to events kan være uenige}
	...
	\todo[inline]{figur helt ens til den overfor men nu med den anden historik ond}
	...
	\todo[inline]{figur: eksempel på hvordan de kan tilføje flere executions sammen}
	...
	\todo[inline]{figur over en order of executions med counters på hver executions - eventuelt en ekstra med hvordan det ser ud efter at x events har executed}
	
	\begin{algorithm}[H]
		\begin{algorithmic}
			\Function{BuildCounterMap}{counterMap}
				\State CounterMap: a map mapping executions to their countervalue.
				\ForAll{Execution from in graph}
					\ForAll{Execution to in from.OutgoingEdges}
						\State counter $\leftarrow$ \Call{Map.Get}{to, CounterMap}
						\State \Call{Map.AddOrUpdate}{to, (counter+1), CounterMap}
					\EndFor
				\EndFor
				\Return CounterMap
			\EndFunction
			...
			\Function{UpdateCountermap}{execution, counterMap}
				\State counterMap $\leftarrow$ \Call{Map.Remove}{execution.Id, counterMap}
				\ForAll{Executions e in action.Edges}
					\State counter $\leftarrow$ \Call{Map.Get}{e, counterMap}
					\State counterMap $\leftarrow$ \Call{Map.AddOrUpdate}{action, counter-1, counterMap}
				\EndFor
				\Return counterMap
			\EndFunction
			...
			\Function{Execute}{state, DCRRules, execution}
				\State conditions $leftarrow$ \Call{List.Where}{id == execution.Id and conditionType, DCRRules}
				\State relations $leftarrow$ \Call{List.Where}{id == execution.Id and not conditionType, DCRRules}
				...
				\EndFor
				\Return \Call{IsExecutable}{state, conditions, execution}
			\EndFunction
			...
			\ForAll{Executions e in Graph}
				\State \Call{Map.AddOrUpdate}{e,0,counterMap}
			\EndFor
			\State failures $\leftarrow$ List.Empty
			\While{not Countermap.isEmpty}
				\State execution $\leftarrow$ \Call{Map.Head}{\Call{Map.Filter}{value=0, counterMap}}
