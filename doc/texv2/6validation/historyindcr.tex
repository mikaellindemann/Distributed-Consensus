...
\chapter{Validation of Histories}\label{chap:validation}
	...
	As argued in \autoref{chap:order-of-execution}, concurrent executions can only be present in the order of execution if the executed events are independent. In \cite{debois2015concurrency}, Debois et. al. conclude that any order of execution of independent events results in the same marking. Therefore any topological ordering of an order of execution is sufficient to simulate any ordering of concurrent events.
    ...
    \newpar We have shown that it is not possible to observe all kinds of consistent cheating, but given an ideal structure of a DCR graph together with a fitting distribution of events then it is possible for the different owners of the workflow, to validate each other and the histories their events provide.